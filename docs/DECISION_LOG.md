...
## ADR-098 — Scan intake requires visible vehicle context

Экран загрузки документа обязан до выбора файла явно показывать автомобиль назначения: марку, модель, маскированный VIN и текущий пробег. Без выбранного автомобиля upload control отключён. Возврат к выбору выполняется через гараж с сохранением активной карточки; скрытый `vehicleId` без пользовательского контекста не считается достаточным UX-контрактом.
