...
- Экран «Скан» теперь явно показывает выбранный автомобиль, маскированный VIN, год и текущий пробег до выбора файла.
- Добавлены действия «Сменить» и «Выбрать в гараже» с восстановлением клавиатурного фокуса.
- Загрузка документа заблокирована, пока автомобиль не выбран.
- Уточнён текст сценария: распознанные данные попадают в историю только после проверки и подтверждения владельцем.
- Добавлены мобильные touch-targets 44 px, общие focus-visible состояния и поддержка reduced motion для нового сценария.
- Добавлены четыре контрактных теста выбранного автомобиля на экране сканирования.
- Канонический релизный `VERSION` остаётся `0.25.0` до восстановления полного зелёного release gate.
